...
def calculate_avg_lecture_grade(lecturers, course):
    total_grade = sum(
        sum(lecturer.lecture_grades[course]) for lecturer in lecturers if hasattr(lecturer, 'lecture_grades') and course in lecturer.lecture_grades
    )
    total_count = sum(
        len(lecturer.lecture_grades[course]) for lecturer in lecturers if hasattr(lecturer, 'lecture_grades') and course in lecturer.lecture_grades
    )
...
reviewer1 = Reviewer('Sam', 'Brown')
reviewer2 = Reviewer('Sara', 'White')
...
# Rate lectures
lecturer1.lecture_grades['Python'] = [8, 9, 7]
lecturer2.lecture_grades['Python'] = [10, 9, 8]
...
print(f'\nСредняя оценка за домашние задания по Python: {python_avg_hw_grade}')

python_lecturers = [lecturer1, lecturer2]
python_avg_lecture_grade = calculate_avg_lecture_grade(python_lecturers, 'Python')
print(f'Средняя оценка за лекции по Python: {python_avg_lecture_grade}')
